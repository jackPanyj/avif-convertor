...
function getSettings() {
    ...
    return {
        ...
        jobs: cfg.get('jobs', 0),
        autoInstallOnMac: cfg.get('autoInstallOnMac', true)
    };
}
function isMac() {
...
function spawnAndCapture(command, args, opts = {}) {
...
function spawnAvifenc(args) {
    ...
    const promise = new Promise((resolve) => {
        ...
        const child = (0, child_process_1.spawn)('avifenc', args, {
            shell: false,
            env: { ...process.env, PATH: getEnhancedPath() }
        });
        childRef = child;
...
async function ensureAvifEnc(output) {
    const settings = getSettings();
    const check = await spawnAndCapture('avifenc', ['--version']);
    if (check.exitCode === 0)
        return true;
    if (!isMac() || !settings.autoInstallOnMac) {
        vscode.window.showErrorMessage("Could not find 'avifenc' (libavif). Install it and try again. macOS: brew install libavif");
        output.appendLine(check.stderr || check.stdout);
        return false;
    }
    const choice = await vscode.window.showWarningMessage("Could not find 'avifenc'. Install via Homebrew now? (brew install libavif)", 'Install', 'Cancel');
    if (choice !== 'Install')
        return false;
...
async function collectFilesFromDir(dirPath, recursive) {
    const out = [];
    ...
    for (const e of entries) {
        ...
        if (e.isDirectory()) {
            if (recursive)
                out.push(...(await collectFilesFromDir(full, true)));
            continue;
        }
        if (e.isFile() && isSupportedFile(full))
            out.push(full);
    }
    return out;
}
...
function resolveOutPath(inputFile, outDir, preserveRootDir) {
...
async function convertOne(inputFile, settings, preserveRootDir, output, token) {
    ...
    if (settings.jobs > 0)
        args.push('-j', String(settings.jobs));
...
async function expandTargets(uris) {
    const settings = getSettings();
    const files = [];
    const rootsForPreserve = new Map();
    ...
    for (const f of unique)
        newMap.set(f, rootsForPreserve.get(f) ?? null);
    return { files: unique, rootsForPreserve: newMap };
}
function activate(context) {
    const output = vscode.window.createOutputChannel('AVIF Convertor');
    context.subscriptions.push(output);
    const disposable = vscode.commands.registerCommand('avifConvert.convert', async (uri, uris) => {
        const targets = (uris && uris.length > 0 ? uris : uri ? [uri] : []).filter(Boolean);
        if (targets.length === 0)
            return;
        output.show(true);
            ...
            const work = async () => {
                while (true) {
                    if (token.isCancellationRequested)
                        return;
                    const myIndex = idx++;
                    if (myIndex >= expanded.files.length)
                        return;
